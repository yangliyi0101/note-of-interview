...
**@ResponseBody**该注解用于将Controller的方法返回的对象，通过适当的**HttpMessageConverter**转换为指定格式后，写入到Response对象的body数据区。而**@RequestBody**该注解用于读取Request请求的body部分数据，使用系统默认配置的HttpMessageConverter进行解析，然后把相应的数据绑定到要返回的对象上，绑定到 controller中方法的参数上。  
